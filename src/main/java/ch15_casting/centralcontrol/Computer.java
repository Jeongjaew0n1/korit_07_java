...
/*
    프로그램 구현 사항
    interface Power를 implement한 다양한 클래스들을 정리할 예정
    예를 들어 Computer / AirConditioner / Speaker 들이 전부 Power를
    implements 하게 되면 이상의 세개의 클래스들은 강제적으로
    On() / Off() 메서드들을 오버라이드 해야합니다.
    ...
    그것을 업캐스팅 했을 때 A가 가지고 있는 부모 메서드를 호출함으로써
    다수의 서로 다른 클래스의 객체들을 통제할 수 있다는 장점 때문에
    업캐스팅을 적용하게 됩니다.
    그리고 다운캐스팅을 해서 각각 클래스의 고유 메서드도 실행시킬 수 있습니다.

    AirConditioner 클래스 생성 Power implements 하고
    에어컨의 전원을 켭니다.
    에어컨의 전원을 끕니다.
    Speaker도 동일하게 적용
